...
public class QuestionButtonView extends GroupedButtonView<BoardInputValue> {

    public QuestionButtonView(ViewListener viewListener, RectF bounds, int color1, int color2, int color3, Bitmap[] innerImages, Context context, Paint paint) {
    ...
    public <V> void draw(Canvas canvas, Paint paint, V currentPressedVal) {
        ...
        if ((currentPressedVal).equals(BoardInputValue.QUESTION_MARK)) {
    ...
    public BoardInputValue getPressedButtonValue() {
        return BoardInputValue.QUESTION_MARK;
    }
}

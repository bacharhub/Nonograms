...
public class ColorfulPuzzleButtonView extends LabeledPicButtonView {

    private final RectF[] innerImageBoundsGrid;
    private final RectF[] pressedInnerImageBoundsGrid;

    public ColorfulPuzzleButtonView(MenuViewListener menuViewListener, RectF bounds, String description, int color1, int color2, int color3, Bitmap[] innerImages, Context context, Paint paint) {
    ...
    public void draw(Canvas canvas, Paint paint) {
        ...
        for(RectF bounds : boundsGrid) {
            canvas.drawBitmap(innerImages[imageIndex++], null, bounds, paint);
        }
    }

    @Override
    public void onButtonPressed() {
        ((MenuViewListener)viewListener).onColorfulPuzzleButtonPressed();
    }
}
